...
export class DataChartComponent2 implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    console.log('showFilters && isUserReportAdmin && chartConfig.id', this.showFilters);
      ...
      this.filtersSubscription = this.filtersFormGroup.valueChanges
        ...
        .subscribe((filters) => {
          ...
          if (this.noResultsFound) {
            this.utilityService.error(this.resourceService.messages.stmsg.m0008);
          }
  ...
  public addChartSummary(): void {
    ...
    if (chartId) {
      ...
      this.utilityService.error('Chart id is not present');
    }
  }
  ...
  public getChartSummary() {
    ...
    if (_.get(this.chartConfig, 'id')) {
      return this.usageService.getLatestSummary({
